# x402 Standard for Agent-to-Agent Diagnostic Exchange (x402-DX)
...
### Diagnostic Response
Captures the core metrics identified in the initial agent diagnostics.
...
## 4. Implementation Example (Diagnostic Analysis)
...
## 5. Paid Diagnostic Access
When an agent calls a peer's `/diagnostic` endpoint, the server may return a `402 Payment Required`.

1.  **Client**: `GET /diagnostic`
2.  **Server**: `402 Payment Required` (Price: 0.001 TEMPO)
3.  **Client**: Pays the invoice via Tempo blockchain.
4.  **Client**: `GET /diagnostic` with payment proof.
5.  **Server**: Returns signed `DiagnosticResponse`.
